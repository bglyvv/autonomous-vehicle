...
export default class ScenarioManager {
  constructor(editor) {
    ...
    window.addEventListener('load',(e)=>{
        document.getElementById(`example-2`).click()
        document.getElementById('scenario-restart').click();
        // document.getElementById('mode-manual').click();
    })

    // window.onload = this._loadScenario(EXAMPLES[2])


    for (let i = 0; i < EXAMPLES.length; i++){
      document.getElementById(`example-${i}`).addEventListener('click', e => {
        console.log(this)
        this._loadScenario(EXAMPLES[i])
        console.log('salam')
      });
      console.log(i)
    }
